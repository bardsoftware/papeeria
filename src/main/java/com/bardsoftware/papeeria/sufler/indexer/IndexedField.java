...
public enum IndexedField {
    ...
    private final Class<? extends Field> myFieldClass;
    private final Field.Store myFieldStore;

    IndexedField(Class<? extends Field> fieldClass, Field.Store fieldStore) {
        myFieldClass = fieldClass;
        myFieldStore = fieldStore;
    }
    ...
    public Field createField(String value) {
        ...
        try {
            ...
            result = (Field) constructor.newInstance(this.name().toLowerCase(), value, myFieldStore);
        } catch (Exception e) {
